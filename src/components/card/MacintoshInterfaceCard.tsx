		...
		<section class="px-4 py-2 border border-muted-background bg-muted-transparent w-full rounded-xl flex flex-col gap-2 lg:gap-4 shadow-lg">
			<div class="w-full flex items-center gap-4 2xl:gap-8 border-b border-muted-background pt-1 pb-2">
				<ul class="w-fit flex items-center gap-2">
					<button type="button" title="Close" onClick={() => setIsOpen(false)} class={`bg-red-500 w-3 h-3 xl:w-4 xl:h-4 rounded-full ${isOpen() ? "" : "opacity-30"}`} />
					<button type="button" title="Close" onClick={() => setIsOpen(false)} class={`bg-yellow-500 w-3 h-3 xl:w-4 xl:h-4 rounded-full ${isOpen() ? "" : "opacity-30"}`} />
					<button type="button" title="Open" onClick={() => setIsOpen(true)} class="bg-green-500 w-3 h-3 xl:w-4 xl:h-4 rounded-full" />
				</ul>
